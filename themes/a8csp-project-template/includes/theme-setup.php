...
function a8csp_template_theme_enqueue_assets(): void {
	...
	if ( null !== $style_meta ) {
		...
		wp_enqueue_style( $style_handle, get_stylesheet_uri(), $style_meta['dependencies'], $style_meta['version'] );
	...
	if ( null !== $script_meta ) {
		wp_enqueue_script( "{$theme_slug}-script", get_theme_file_uri( $script_path ), $script_meta['dependencies'], $script_meta['version'], true );
	}
}
