...
final class BookCountBlockTest extends \PHPUnit\Framework\TestCase {
	...
	/**
	 * Confirms the block renders the published Book count as a link to the Book archive.
	 *
	 * @return  void
	 */
	public function test_book_count_block_renders_the_count_linked_to_the_archive(): void {
		...
		$book_count = (int) wp_count_posts( 'a8csp_template_book' )->publish;
		...
		self::assertStringContainsString( '<a href="' . esc_url( (string) get_post_type_archive_link( 'a8csp_template_book' ) ) . '">' . number_format_i18n( $book_count ) . ' book', $output );
	}
