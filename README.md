...
- `themes/a8csp-project-template/` is the worked block theme. `functions.php` defines a shared asset-metadata helper and a `sort()`-ordered, underscore-opt-out loader for `includes/`; `includes/theme-setup.php` registers theme support and enqueues `style.css` (built from `assets/sass/`) plus the built JS entry in `assets/js/`.
...
- `tests/` is one whole-site honeycomb, not one suite per component: `tests/Integration/` (plain PHPUnit `TestCase`, run inside the tests wp-env instance) and `tests/EndToEnd/` (Playwright, against the dev wp-env instance). See `tests/README.md`.
